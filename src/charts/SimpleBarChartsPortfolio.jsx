  ...
  return (
    <>
      ...
      <section className='charts-button-download'>
        <ButtonDownloadImg title='Descargar grafica' fileName='Gestión de Portafolio por Asesor' screenSize={screenSize} containerRef={containerRef} date={`${dia} ${mes}`} />
      </section>
      <section ref={containerRef}>
        <h4 className='text-center mt-2'>{(dia !== undefined && mes !== undefined) ? (`Gestión de Portafolio por Asesor ${dia} ${mes}`) : 'Gestión de Portafolio por Asesor'}</h4>
        <div className='simple-bar-charts'>
          <ResponsiveContainer width={screenSize} aspect={2}>
            <ComposedChart data={leakedData} margin={{ top: 50, bottom: 95 }}>
              ...
              <Bar name='% Clientes atendidos del portafolio' dataKey='porcentajeClientesAtendidosDelPortafolio' fill='#4673bf'>
                <LabelList dataKey='porcentajeClientesAtendidosDelPortafolio' position='top' fill='#7d7a79' formatter={customTooltipFormatter} />
              </Bar>

              <Bar name='% Ventas del portafolio' dataKey='porcentajeVentasPortafolio' fill='#fafa0c' color='black'>
                <LabelList dataKey='porcentajeVentasPortafolio' position='top' fill='#7d7a79' formatter={customTooltipFormatter} />
              </Bar>
